...
import { API } from '../App';
import { FolderKanban, Users, Clock, TrendingUp, AlertCircle, CheckCircle, ChevronDown, ChevronUp, Search } from 'lucide-react';
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6" data-testid="projects-module">
      <div>
        ...
        <p className="text-[#4A5B6E] text-sm mt-1">Active delivery portfolio — March 2026</p>
      </div>
      ...
      <div className="bg-[#0A1628] border border-[#1B2D42] rounded-lg overflow-hidden">
        <div className="p-4 border-b border-[#1B2D42]">
          <h2 className="text-sm font-bold text-[#E8EDF2]">Project Health Dashboard</h2>
        </div>
              ...
              {healthData.map(h => (
                <tr key={h.id} className="border-b border-[#1B2D42]/50 hover:bg-[#152236]/50 cursor-pointer transition-colors"
                  ...
                  <td className="px-3 py-2.5">
                    <span className="font-bold text-[#E8EDF2]">{h.id}</span>
                    <span className="text-[#4A5B6E] ml-1.5">{h.name}</span>
                  </td>
                  <td className="px-3 py-2.5 text-[#4A5B6E]">{h.client}</td>
                  <td className="px-3 py-2.5">
                    <span className="px-2 py-0.5 rounded text-[9px] font-bold" style={{ background: `${TYPE_COLORS[h.type] || '#6b7280'}18`, color: TYPE_COLORS[h.type] || '#6b7280' }}>{h.type}</span>
                  </td>
                  <td className="px-3 py-2.5">
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold" style={{ background: `${HEALTH_COLORS[h.health] || '#6b7280'}18`, color: HEALTH_COLORS[h.health] || '#6b7280' }}>
                      <span className="w-1.5 h-1.5 rounded-full" style={{ background: HEALTH_COLORS[h.health] || '#6b7280' }} />
                      {h.health}
                    </span>
                  </td>
                  <td className="px-3 py-2.5">
                    {h.pct_complete != null ? (
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-[#1B2D42] rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${h.pct_complete}%`, background: HEALTH_COLORS[h.health] || '#38bdf8' }} />
                        </div>
                        <span className="text-[#E8EDF2] font-bold">{h.pct_complete}%</span>
                      </div>
                    ) : <span className="text-[#4A5B6E]">{h.status}</span>}
                  </td>
      ...
      {selectedProject && (
        <div className="bg-[#0A1628] border border-[#1B2D42] rounded-lg p-4 space-y-4" data-testid="project-detail">
          ...
          {(() => {
            ...
            if (!proj?.milestones) return null;
            return (
              <div>
          ...
          <div key={p.id} className="bg-[#0A1628] border border-[#1B2D42] rounded-lg p-4" data-testid={`project-card-${p.id}`}>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-[10px] text-[#4A5B6E] mt-0.5">
                  Team: {p.team_names?.join(', ')}
                </p>
              </div>
