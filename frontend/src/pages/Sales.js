  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
        <h1 className="heading-font text-4xl font-black tracking-tighter text-slate-900">Sales</h1>
            ...
            return (
              <button
                ...
                className={`px-4 py-3 font-medium flex items-center space-x-2 border-b-2 ${
                  activeTab === tab.id ? 'border-[#002FA7] text-[#002FA7]' : 'border-transparent text-slate-600'
                }`}
              >
                <Icon size={16} />
                <span>{tab.label}</span>
          ...
          {activeTab === 'quotations' && (
            quotations.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-500">No quotations</p>
                <p className="text-sm text-slate-400 mt-1">Use AI: "Create quotation for ABC Corp - 100 laptops"</p>
              </div>
            ) : (
              <div className="space-y-4">
                {quotations.map((quot) => (
                  <div key={quot.id} className="border border-slate-200 p-4 rounded-sm">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-slate-900">{quot.customer_name}</h3>
                        <p className="text-sm text-slate-600">{quot.transaction_date}</p>
                        <p className="text-lg font-bold text-[#002FA7] mt-2 mono">₹{quot.grand_total?.toLocaleString()}</p>
                      </div>
                      <div className="space-y-2">
                        <span className={`px-3 py-1 rounded text-xs font-medium ${
                          quot.status === 'Draft' ? 'bg-gray-100 text-gray-700' :
                        ...
                        {quot.status === 'Submitted' && (
                          <button
                            ...
                            className="block w-full text-xs px-3 py-1 bg-[#002FA7] text-white rounded-sm"
                          >
                            Convert to SO
                          </button>
                        )}
          ...
          {activeTab === 'sales-orders' && (
            salesOrders.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-500">No sales orders</p>
              </div>
            ) : (
              <div className="space-y-4">
                {salesOrders.map((so) => (
                  <div key={so.id} className="border border-slate-200 p-4 rounded-sm">
                    <h3 className="font-bold text-slate-900">{so.customer}</h3>
                    <p className="text-sm text-slate-600">Delivery: {so.delivery_date || 'TBD'}</p>
                    <div className="mt-2 flex items-center space-x-4 text-xs">
                      <span className="mono">Qty: {so.total_qty}</span>
                      <span className="mono font-bold text-[#002FA7]">₹{so.grand_total?.toLocaleString()}</span>
            ...
            deliveryNotes.length === 0 ? (
              ...
              <div className="space-y-4">
                {deliveryNotes.map((dn) => (
                  <div key={dn.id} className="border border-slate-200 p-4 rounded-sm">
                    <h3 className="font-bold text-slate-900">{dn.customer}</h3>
                    <p className="text-sm text-slate-600">{dn.posting_date}</p>
